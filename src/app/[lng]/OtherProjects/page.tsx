...
export async function generateMetadata({ params }: OtherProjectProps): Promise<Metadata> {
  return {
    title: `Game projects`,
  };
}
...
export default async function OtherProjects({
  ...
  return (
    ...
    <div className={styles.projectContainer}>
      <h2 className={`${styles.b} ${styles.title}`}>{t("whatIDO.otherProjectPageTitle")}</h2>
      <div className={styles.projects}>
        {projectData.whatIDO &&
          projectData.whatIDO.projects &&
          OtherProjects.map((project: any, index: number, selected: boolean) => (
